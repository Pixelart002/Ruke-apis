...
async def choose_tool_and_generate_chips(user_prompt: str, history: List[Dict]) -> Dict:
    ...
    if not decision.get("tool_name") or decision.get("tool_name") not in TOOL_REGISTRY:
        ...
        else: decision["tool_name"] = "creative_writing_mistral"
    if not decision.get("prompt_for_tool"): decision["prompt_for_tool"] = user_prompt
    if not decision.get("chips"): decision["chips"] = [f"Explain '{user_prompt.split()[-1]}' more", "Generate an image for this"]
    return decision
...
@router.post("/ask")
async def ask_yuku_mcp(prompt: str = Form(...), chat_id: str = Form(...), tool_override: Optional[str] = Form(None), file: Optional[UploadFile] = File(None), current_user: Dict = Depends(auth_utils.get_current_user)):
    if file:
    ...
    async with httpx.AsyncClient(timeout=120.0) as client:
        try:
            ...
            if tool["method"] == "POST":
                if "message" in params: # Midjourney
            ...
            if tool.get("response_is_image_url"):
                ...
                if chosen_tool == "code_generation" and content.strip().startswith("{") and '"index.html"' in content:
